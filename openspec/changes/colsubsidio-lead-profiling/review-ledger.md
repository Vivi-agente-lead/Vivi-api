...
| CODE-002 | readability | `app/core/config.py:56-57` | WARNING | fixed | `whatsapp_api_version` declared twice — closed by `cb005f9` (task 0.3) |
| P0-1 | resilience | `requirements.lock` (last line) | BLOCKER | open | Lock ends in `-e git+ssh://…@52c7e7c#egg=vivi_api`: installs a pre-Phase-0 copy of the app over SSH; the Fly Docker build has no key |
| P0-2 | reliability | `requirements.lock:31` ↔ `pyproject.toml` | CRITICAL | open | `openpyxl==3.1.5` is locked but undeclared, so `pip install .` does not reproduce the lock as the manifest comment claims |
| P0-3 | reliability | `pyproject.toml:13-18` + `openspec/config.yaml:9-11` | CRITICAL | open | Both claim the pins were resolved "against the committed `.venv`"; `.venv/` is gitignored and absent, so the lock cannot be reproduced or regenerated |
| P0-4 | reliability | `scripts/_verify_langgraph_api.py:81-93` | CRITICAL | open | Verifies `add_conditional_edges(src, fn, path_map)`; `design.md` §3 and task 4.2 use the two-arg form. The form actually shipped was never covered |
| P0-5 | reliability | `scripts/_verify_langgraph_api.py:38` | CRITICAL | open | `assert END != "END"` compares a constant to a literal and proves nothing about router behaviour; a router returning `"END"` is accepted **silently** |
| P0-7 | readability | `design.md:112` | SUGGESTION | open | §3 still says the pin "MUST" be added and the API "is to be re-verified … as the first task of the apply phase"; both are done |
| P0-8 | resilience | `requirements.lock` + `Dockerfile:2` | SUGGESTION | open | `pip freeze` records no interpreter; the lock was built on 3.14 while the image is `python:3.12-slim` and `config.yaml` says 3.12 |
...
## Appendix E — Phase 0 review (2026-07-26)

Phase 0 landed in PR #2 (`507965a`, `a5e3ed1`, `cb005f9`). Reviewed against the
installed package, not against the diff.

### Verified working

- **0.3** — duplicate `whatsapp_api_version` removed, comment indentation fixed.
  `pytest -q` → 4 passed. `CODE-002` closed.
- **0.1** — all 15 pins queried against PyPI: every one exists, and every one is the
  latest published release. `pyproject.toml` converted from `>=` to `==` correctly.
- **0.2** — `scripts/_verify_langgraph_api.py` was executed against a clean
  `langgraph==1.2.9` install. Exit 0. Confirmed: `END == "__end__"`,
  `create_react_agent` still resolves (no `builder.py` shim needed before Phase 4),
  async partial-state deltas merge, `compile(checkpointer=…)` returns a runnable with
  `ainvoke`/`astream`.

### Correction to SDD-005

The original audit could not check whether the versions in `openspec/config.yaml`
existed, and marked the question `UNVERIFIED`. They exist and are current. The finding
that survives is only the one originally stated — the declared versions contradicted
`pyproject.toml`. Any framing of those numbers as fabricated was wrong.

### Independent checks Phase 0 did not perform

Run here, against `langgraph==1.2.9`:

- **Two-arg `add_conditional_edges(src, router_fn)`** — accepted. A predicate returning
  the `END` sentinel terminates correctly (`edad=17` → terminates, `edad=None` →
  terminates, `edad=30` → reaches the next node). `design.md` §3 is sound; P0-4 records
  that Phase 0 verified a different form.
- **A router returning the literal `"END"`** — accepted **silently**. langgraph emits
  `Task gate … wrote to unknown channel branch:to:END, ignoring it.` and the graph
  terminates normally with no exception. The original `LOGIC-002` defect would have been
  invisible in production: the consent opt-out and both underage gates would have
  appeared to work while doing nothing.
  **Consequence for task 4.3**: assert a *traversal*, not a sentinel value. Comparing
  `END` to `"END"` cannot catch this class of defect, and langgraph will not raise.
- **`pip install -r requirements.lock`** — succeeds locally only because an SSH key is
  loaded; it clones the repo at `52c7e7c` into `src/vivi-api`. See P0-1.
